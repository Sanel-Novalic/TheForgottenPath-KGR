...
namespace SG
{
	public class MazeLoader : MonoBehaviour
	{
		...
		private List<GameObject> InstantiatedCheckPoints { get; set; }

		void Start()
		{
			InstantiatedCheckPoints = new List<GameObject>();
		...
		{
			var instantiated = Instantiate(CheckBox, new Vector3(CurrentRow * size + offset, -(size / 2f), CurrentColumn * size), Quaternion.identity);
			InstantiatedCheckPoints.Add(instantiated);
		...
		public void CreateRespawnPosition(int CurrentRow, int CurrentColumn, float Offset = 0f)
		{
			...
			InstantiatedCheckPoints[InstantiatedCheckPoints.Count - 1].GetComponent<Checkpoints>().SetRespawnPosition(position);
		}
